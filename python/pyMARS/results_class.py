...
class data():
    ...
    def plot_BnPEST(self, ax, n=2, inc_contours = 1, contour_levels = None, phase = 0, increase_grid_BnPEST = 0, min_phase = -130, max_ss = 1.0, interp_points = 100, gauss_filter = [0,0.5], cmap = 'hot', n_contours = 5, phase_ref=None, rmax = 3, phase_ref_array = None, sqrt_flux = True):
        ...
        if phase_ref!=None:
            ...
            part1 = self.BnPEST[:ss_plas_edge,:]
            part1 = self.BnPEST[:ss_plas_edge,:]/((phase_ref_array/np.abs(phase_ref_array))[:ss_plas_edge,:])
            tmp_plot_quantity = 1.*((1.*(part1).transpose()/(self.A[:ss_plas_edge]/(4*np.pi**2))).transpose())


            #tmp_plot_quantity_real = 1.*((1.*(self.BnPEST[:ss_plas_edge,:]).transpose()/(self.A[:ss_plas_edge]/(4*np.pi**2))).transpose()) * np.exp(1j*np.deg2rad(tmp_phpase_ref))
            #tmp_plot_quantity[tmp_phase_ref<180]*=-1.
        elif phase==1:
        ...
        if increase_grid_BnPEST:
            #tmp_plot_quantity2 = complex_array_to_rgb(tmp_plot_quantity_real + 1j * tmp_plot_quantity_imag, rmax=1, theme = 'dark')
            tmp_plot_quantity2 = tmp_plot_quantity if not phase_ref else tmp_plot_quantity_rgb
            color_ax = ax.imshow(tmp_plot_quantity2, cmap=cmap, aspect='auto', interpolation='bicubic', rasterized=True, extent = [np.min(tmp_plot_mk), np.max(tmp_plot_mk), np.min(tmp_plot_ss), np.max(tmp_plot_ss)], origin='lower')
        else:
        ...
        if inc_contours:
            ...
            if contour_levels==None:
                ax.contour(tmp_plot_mk,tmp_plot_ss, tmp_contour_plot, n_contours, colors='white')
            else:
                ax.contour(tmp_plot_mk,tmp_plot_ss, tmp_contour_plot, contour_levels, colors='white')
