...
class dBres_calculations():
    def __init__(self, project_dict):
        self.project_dict = project_dict
        self.raw_data = {}
        for field in ['total', 'vacuum']:
            for coil in ['upper','lower']:
                self.raw_data['{}_res_{}'.format(field, coil)] = data_from_dict('responses/{}_resonant_response_{}'.format(field,coil), self.project_dict)
...
class dBkink_calculations():
    def __init__(self, project_dict, s_surface, n, phase_machine_ntor, reference_offset, ref = 'plasma'):
        ...
        #get data for the reference harmonic
        self.raw_data['reference'] = get_reference(self.raw_data['{}_kink_upper'.format(ref)], self.raw_data['{}_kink_lower'.format(ref)], np.linspace(0,2.*np.pi,100), self.n, phase_machine_ntor = self.phase_machine_ntor)

        #Find the actual harmonic
        for field in ['total', 'vacuum', 'plasma']:
            for coil in ['upper','lower']:
                self.raw_data['{}_kink_harm_{}'.format(field, coil)], mode_list, max_mode_list = calculate_db_kink2(self.raw_data['mk'], self.raw_data['q_val'], self.n, self.raw_data['reference'], self.raw_data['{}_kink_{}'.format(field,coil)], reference_offset = self.reference_offset)
        ...
        self.phasing_scan(n_phases = 360, phasing_array = None, field = 'total')

    def apply_phasing(self, phasing, field = 'total'):
        return apply_phasing(self.raw_data['{}_kink_harm_upper'.format(field)], self.raw_data['{}_kink_harm_lower'.format(field)], phasing, self.n, phase_machine_ntor = self.phase_machine_ntor)
    ...
    def phasing_scan(self, n_phases = 360, phasing_array = None, field = 'total'):
        ...
        output_array = np.zeros((len(phasing_array), len(self.raw_data['mk'])), dtype=complex)
        for i, curr_phase in enumerate(phasing_array):
            output_array[i,:] = self.apply_phasing(curr_phase, field = field)
...
class test1_new():
    def __init__(self, file_name, s_surface, phasing, phase_machine_ntor, fixed_harmonic = 5, reference_offset = None, reference_dB_kink='plas',sort_name = 'q95_list', try_many_phasings = True):
        ...
        self.tmp_dict = {}
        for i in plasma_params:self.tmp_dict[i] = data_from_dict(i, self.project_dict)

        #MARS_settings
        plasma_params = ['ROTE','ETA']
        for i in plasma_params:self.tmp_dict[i] = data_from_dict('MARS_settings/<<{}>>'.format(i), self.project_dict)
        self.dBres = dBres_calculations(self.project_dict)
        self.dBkink = dBkink_calculations(self.project_dict, self.s_surface, self.n, self.phase_machine_ntor, self.reference_offset, ref = reference_dB_kink)
        #dBres calculations
                
        #shot_time = self.data_from_dict('shot_time')
        #ROTE = self.data_from_dict('MARS_settings/<<ROTE>>')
        #ETA = self.data_from_dict('MARS_settings/<<ETA>>')

        #tot_res_upper = self.data_from_dict('responses/total_resonant_response_upper')
