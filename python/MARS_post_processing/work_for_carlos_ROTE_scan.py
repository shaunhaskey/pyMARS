...
file_name = '/home/srh112/NAMP_datafiles/mars/shot158115_04780_scan_ideal/shot156910_03517_single_carlos_ROTE_scan/'
...
def extract_useful(file_name, m = 10, probe_names = None):
    ...
    vtor = np.array(dBres.parent.raw_data['ROTE'])/(np.array(dBres.parent.raw_data['R0EXP'])/np.array(dBres.parent.raw_data['v0a']))
    return results, phases, betaN_li, vtor/1000, np.array(dBres.parent.raw_data['Q95'])
...
##########  ROTE SCAN FOR CARLOS ~1 OCT 2014 ###############
...
print dirs
dirs = ['shot156910_03517_single_carlos_ROTE_scan']
...
for file_name, label, marker in zip(file_names, labels, markers):
    results, phases, betaN_li, rote, q95 = extract_useful(file_name, m = 10, probe_names = probe_names)
    forced_phase = 0
    ...
    for i, clr in zip(keys, colour):
        ...
        print i
        max_phases, max_amps, max_angs = max_amp_max_phase(results['probe'], i, 'plasma', phases, ax1 = ax[0], ax2 = ax2, ax3 = ax3, x_ax = rote, plot_kwargs = plot_kwargs, forced_phase = forced_phase, print_vals = True)
...
ax[-1].set_xlabel('vtor 0 (krad/s)')
ax[0].set_ylabel('G/kA')
for i in ax: i.grid()
if ax[1]!=None: ax[1].set_ylabel('Signal Phase (deg)')
if ax[2]!=None: ax[2].set_ylabel('UL phasing (deg)')
...
##########  Q95 SCAN FOR CARLOS 5 OCT 2014 ###############
dirs = ['shot158117_03885_q95_scan_carlos_q95_scan']
dirs = ['shot153585_03795_q95_scan_josh_q95_scan']
#file_names = ['/home/srh112/NAMP_datafiles/mars/shot156746_02113_betaN_ramp_carlos_prl2']
file_names = []
probe_names = ['66M', 'MPID1A', 'MPID1B']
colour = ['b','r','k']
...
for file_name, in zip(file_names,):
    ...
    for i, clr in zip(keys, colour):
        ...
        #plot_kwargs = {'marker': 'o', 'color':clr, 'label' :'{}'.format(i.replace('_','\_'))}
        print i
        max_phases, max_amps, max_angs = max_amp_max_phase(results['probe'], i, 'plasma', phases, ax1 = ax[0], ax2 = ax2, ax3 = ax3, x_ax = q95, plot_kwargs = plot_kwargs, forced_phase = forced_phase, print_vals = True)
        #lab = '{} - {}'.format(i,'ideal MHD')
        #max_phases, max_amps, max_angs = max_amp_max_phase(results['probe'], i, 'plasma', phases, ax1 = ax[0], ax2 = ax2, ax3 = ax3, x_ax = betaN_li2, plot_kwargs = plot_kwargs, forced_phase = forced_phase)
    #ax[0].set_xlim([0.5,3.177])
ax[0].set_xlim([3.4,4.2])
ax[0].set_ylim([0,3.5])
ax[0].set_ylim([0,15.])
ax[1].set_ylim([-180,180])
ax[-1].set_xlabel('q95')
ax[0].set_ylabel('G/kA')
for i in ax: i.grid()
if ax[1] is not None: ax[1].set_ylabel('Signal Phase (deg)')
if ax[2] is not None: ax[2].set_ylabel('UL phasing (deg)')
for i in ax: ax[0].set_ylabel('G/kA')
leg = ax[0].legend(loc = 'best', fontsize = 8)
fig.canvas.draw(); fig.show()
