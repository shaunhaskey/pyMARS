...
keys = [int(sys.argv[i]) for i in range(2,len(sys.argv))]
...
for i in keys:
    ...
    im_name_list.append('{}/TIME_{:05d}_ROTE_{:010d}_ETA_{:010d}'.format(base_dir, scan_data['sims'][i]['shot_time'], int(scan_data['sims'][i]['MARS_settings']['<<ROTE>>']*10**10), int(scan_data['sims'][i]['MARS_settings']['<<ETA>>']*10**10)))
    title_list.append('TIME {} ROTE {:.3e} ETA {:.3e}'.format(scan_data['sims'][i]['shot_time'], scan_data['sims'][i]['MARS_settings']['<<ROTE>>'], scan_data['sims'][i]['MARS_settings']['<<ETA>>']))
print im_name_list
