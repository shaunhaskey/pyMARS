...
gen_func.setup_publication_image(fig, height_prop = 1./1.618*2, single_col = True)
...
ax[2].plot([t for t, res in tmp], [res for t, res in tmp])
...
ax[3].plot([t for t, kink in tmp], [kink for t, kink in tmp])
ax[3].set_ylabel('dBkink')

tmp = np.sort([[t, q] for t, q in zip(a.time_list, a.q95_list)],axis = 0)
ax[4].plot([t for t, q in tmp], [q for t, q in tmp])
ax[4].set_ylabel('q')
